...
class FullVlm26MemoryPlanner(SyncLoRAPlanner):
    ...
    def infer_sync(self, step_idx: int, context_frames_np: list[tuple[np.ndarray, np.ndarray | None]]) -> str:
        ...
        if hidden_steps:
            # The first generation step contains the complete causal prompt. Its
            # final-layer, final-token state is the online upper-tower retrieval feature.
            first_step = hidden_steps[0]
            last_layer = first_step[-1] if isinstance(first_step, (tuple, list)) else first_step
            feature = last_layer[0, -1].detach().float().cpu().numpy()
            if feature.ndim != 1 or not np.isfinite(feature).all():
                raise RuntimeError(f"Invalid upper VLM feature shape: {feature.shape}")
            self._latest_feature = feature
            self._latest_feature_step = int(step_idx)
...
def run_episode_async_stateful(
    *,
    ...
    subtask_buffer = {"value": "", "step_idx": -1, "feature": None}
    ...
    def write_subtask(step_idx: int, subtask: str) -> None:
        with subtask_lock:
            ...
            subtask_buffer["feature"] = (
                None if planner._latest_feature is None else planner._latest_feature.copy()
            )

    def read_subtask() -> tuple[str, int, np.ndarray | None]:
        with subtask_lock:
            feature = subtask_buffer["feature"]
            return (
                str(subtask_buffer["value"]),
                int(subtask_buffer["step_idx"]),
                None if feature is None else feature.copy(),
            )
    ...
    try:
        ...
        while t < args.max_steps + args.num_steps_wait:
            ...
            if args.async_vlm:
                ...
                latest_subtask, latest_step, latest_feature = read_subtask()
            else:
            ...
            element["progress"] = float(np.clip(effective_t / max(args.max_steps, 1), 0.0, 1.0))
            element["upper_vlm_available"] = latest_feature is not None
            element["upper_vlm_age"] = float(
                np.clip((effective_t - latest_step) / max(args.vlm_interval * 4, 1), 0.0, 1.0)
            )
            if latest_feature is not None:
                element["upper_vlm_feature"] = latest_feature
