...
RUN_ROOT="${RUN_ROOT:-${OPENPI_ROOT}/logs/pi_v0_positive_four_suites_seed7_$(date -u +%Y%m%d_%H%M%S)}"
GPU="${GPU:-0}"
PORT="${PORT:-8200}"
SEED="${SEED:-7}"
...
for path in "${POLICY_DIR}/model.safetensors" "${TASK_HEAD_CKPT}" "${MEMORY_META_PATH}" \
  ...
  [[ -f "${path}" ]] || { echo "Missing V0+ artifact: ${path}" >&2; exit 1; }
done
...
echo "Pi V0 positive-only four-suite evaluation complete: ${RUN_ROOT}"
