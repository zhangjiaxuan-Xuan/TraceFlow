...
# Short Full26 validation for the fitted two-metric gate: 10 episodes/task.
# v1.1 = Fusion-V1 guidance + offline fitted separability/alignment gate.
...
SEED="${SEED:-50}"
RUN_ROOT_BASE="${RUN_ROOT_BASE:-/path/to/storage/datasets/robotics/RoboMemArena/derived/retrieval_full26_tdense5/eval_v1_1_short}"
RUN_ROOT="${RUN_ROOT:-${RUN_ROOT_BASE}/v1_1_full26_ep${EPISODES_PER_TASK}_seed${SEED}_$(date -u +%Y%m%d_%H%M%S)}"
...
export RECORD_MEMORY_DATA="${RECORD_MEMORY_DATA:-1}"
export MEMORY_TRACE_LEVEL="${MEMORY_TRACE_LEVEL:-light}"
export PREDIMEM_COMPONENT_TIMING="${PREDIMEM_COMPONENT_TIMING:-1}"

# The downstream full26 gate wrapper accepts the task subset and short-run
# settings through the environment while keeping the official server path.
