...
SOURCE_ROOT="${SOURCE_ROOT:-/path/to/storage/datasets/robotics/RoboMemArena/derived/retrieval_self_v4_dense10/pi05_finetuned}"
MANIFEST="${MANIFEST:-${SOURCE_ROOT}/features/anchors_stride10.jsonl}"
FEATURE_DIR="${FEATURE_DIR:-${SOURCE_ROOT}/features/lower}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/path/to/storage/datasets/robotics/RoboMemArena/derived/retrieval_self_v4_dense10_single_frame/pi05_finetuned}"
...
for path in \
  ...
  [[ -e "${path}" ]] || { echo "Missing completed dense10 source artifact: ${path}" >&2; exit 2; }
done
...
print(
    f"Verified reusable dense10 cache: rows={rows} source_dim=6144 "
...
if [[ "${PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "Pi dense10 single-frame head preflight passed: ${ARTIFACT_ROOT}"
  exit 0
fi
...
echo "[stage 1/2] Train single-frame 2048-1024-2048 retrieval head"
...
echo "[stage 2/2] Build dense-frame metadata and enforce the identical temporal gate"
...
print(
    f"Single-frame dense temporal gate: task_stage_R@1={recall:.6f} "
    f"frame_MAE={mae:.3f} frame_P95={p95:.3f}",
    flush=True,
)
if recall < 0.99 or mae > 6.0 or p95 > 6.0:
    raise SystemExit(
        "Single-frame dense temporal gate failed; artifacts were preserved for ablation analysis."
    )
PY

echo "Pi0.5 dense10 single-frame head complete: ${ARTIFACT_ROOT}"
