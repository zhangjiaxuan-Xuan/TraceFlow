...
OUTPUT_ROOT="${OUTPUT_ROOT:-/path/to/storage/datasets/robotics/RoboMemArena/derived/retrieval_self_v5_dense5_budget/pi05_finetuned}"
MANIFEST="${MANIFEST:-${OUTPUT_ROOT}/features/anchors_stride5.jsonl}"
FEATURE_DIR="${FEATURE_DIR:-${OUTPUT_ROOT}/features/lower}"
ARTIFACT_ROOT="${OUTPUT_ROOT}/budget1024_fp32"
DENSE10_ROOT="${DENSE10_ROOT:-/path/to/storage/datasets/robotics/RoboMemArena/derived/retrieval_self_v4_dense10/pi05_finetuned}"
DENSE10_MANIFEST="${DENSE10_MANIFEST:-${DENSE10_ROOT}/features/anchors_stride10.jsonl}"
DENSE10_FEATURE_DIR="${DENSE10_FEATURE_DIR:-${DENSE10_ROOT}/features/lower}"
...
for path in \
  ...
  "${DENSE10_MANIFEST}" \
  "${DENSE10_FEATURE_DIR}/pooled_prefix.npy" \
  "${DENSE10_FEATURE_DIR}/completed.npy" \
  "${DENSE10_FEATURE_DIR}/cache_state.json"; do
...
for action_id, items in by_action.items():
    ...
    assert all(0 < right - left <= 5 for left, right in zip(frames, frames[1:]))
    for row in items:
...
print(f"Verified dense5 temporal manifest: trajectories={len(by_action)} rows={len(rows)}")
PY

if [[ "${PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "Pi dense5 budget1024 training preflight passed: ${OUTPUT_ROOT}"
  exit 0
fi
...
echo "[stage 1/3] Resume dense5 three-frame feature cache"
"${OPENPI_PY}" "${ROOT}/scripts/memory/cache_prior_head_features.py" \
  ...
  --feature-dim 2048 \
  --seed-manifest "${DENSE10_MANIFEST}" \
  --seed-cache-dir "${DENSE10_FEATURE_DIR}"
...
print(f"Verified complete dense5 temporal feature cache: {count}/{len(completed)}", flush=True)
PY

echo "[stage 2/3] Train stage-relative 6144-1024-1024 retrieval head"
...
echo "[stage 3/3] Build dense-frame metadata and report temporal diagnostics"
...
echo "Pi0.5 dense5 budget1024 self-head complete: ${ARTIFACT_ROOT}"
