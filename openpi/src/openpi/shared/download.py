...
def _download_gsutil(url: str, local_path: pathlib.Path, **kwargs) -> None:
    ...
    is_google_gsutil = False
    if executable is not None:
        probe = subprocess.run(
            [executable, "version", "-l"],
            capture_output=True,
            text=True,
            check=False,
        )
        identity = (probe.stdout + probe.stderr).lower()
        is_google_gsutil = probe.returncode == 0 and "gsutil version" in identity
    if not is_google_gsutil:
        logger.warning(
            "Google Cloud gsutil not found, falling back to gcsfs."
        )
